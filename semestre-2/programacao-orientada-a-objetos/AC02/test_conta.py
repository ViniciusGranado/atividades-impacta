...
@pytest.mark.parametrize('atributo', ['_Conta__titular', '_Conta__agencia',
                         '_Conta__numero', '_Conta__saldo', '_Conta__ativa',
                            '_Conta__operacoes'])
...
def test_transferir(valor, checa_extrato, ativa_origem, ativa_destino):
	...
	try:
		...
		if not ativa_origem or not ativa_destino:
			...
			assert info_deposito == ('deposito', valor), 'Ao transferir, não use o atributo privado saldo da conta_destino. Use o método depositar(valor) deste objeto'
			if checa_extrato:
